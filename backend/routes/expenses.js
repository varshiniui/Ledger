...
// POST /api/expenses/submit — full pipeline: upload -> OCR -> parse -> save
router.post('/submit', upload.single('receipt'), async (req, res) => {
  try {
    ...
    // 2. OCR — preprocess with sharp for better accuracy
    ...
    if (!extractedText.trim()) {
      return res.status(200).json({
        error: 'No text detected in receipt. Try a clearer photo.',
      });
    }

    // 3. Parse (regex for numbers, Groq for judgment fields)
    ...
    // 4. Duplicate hash: same employee + amount + date + merchant = likely duplicate
    ...
    // 5. Check if this exact hash already exists for this employee
